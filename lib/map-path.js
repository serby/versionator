...
function makeHashMap(files, dirPath, callback) {
  ...
  async.each(
    files,
    function(filename, fileCallback) {
      // resolve '../'s
      filename = path.resolve(filename)

      hash(filename, function(error, fileHash) {
        var urlPath = filename.substring(dirPath.length + 1)
        var normalizedPath = path.normalize('/' + urlPath).replace(/\\/g, '/')
        var pos = normalizedPath.lastIndexOf('/')

        hashMap[normalizedPath] =
          normalizedPath.substring(0, pos) +
            '/' +
            fileHash +
...
module.exports = function(dirPath, params, callback) {
  ...
  var options = Object.assign({ basePath: '', hash: hash }, params)
